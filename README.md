...
Evidence declarations and accepted payment/payout evidence attempts are sticky safety requirements. CreditSync records the requirement before signing, download, DNS, or storage work, retains it when an attempt fails or is cleaned up, and requires every declared attachment to have an exact tenant-scoped finalized association before preview, posting, or activation. When a legacy finalized association is first encountered, only its provable SHA-256 or source-fingerprint identity is seeded into the durable floor; ambiguous identities are never inferred. A read-only workflow resolver can explain the next step, but it never authorizes a financial transition; if a host never reports an attachment, the backend cannot infer that file exists.
