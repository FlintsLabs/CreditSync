...
- reconcile approved historical `needs_review` allocations or restore a fully reversed evidence-backed source through `payment.reconcile.preview` / `payment.restore.preview` → exact review → explicit confirmation → matching execute; restore returns source/child UUID lineage, copies the immutable original component split, and never copies bank-reference or evidence data
