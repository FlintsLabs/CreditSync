...
- reconcile approved historical `needs_review` allocations or repost a fully reversed evidence-backed source through `payment.reconcile.preview` → explicit confirmation → `payment.reconcile.execute`; the latter returns source/child UUID lineage and posts interest only without copying evidence or reducing principal
