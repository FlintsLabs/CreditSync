...
Settlement and renewal execution also stop when the source loan has a known draft payout requirement that is pending or rejected. Restructure follows the same source-loan check before creating replacement financial records. These alternate flows do not invent an attachment transport; unsupported evidence association remains human-review-only.
