...
- reconcile approved historical `needs_review` allocations or restore a fully reversed evidence-backed source through `payment.restore.create` → `payment.restore.evidence.prepare` → signed PUT → `payment.restore.evidence.finalize` → `payment.restore.preview` → exact review → explicit confirmation → `payment.restore.execute`; ordinary payment posting also requires the no-write preflight against the latest ready proposal. Restore returns source/child UUID lineage, copies the immutable original component split, and never copies bank-reference or evidence data
