...
Scheduled-loan replacement is available only to tenant owners and managers. It is `preview -> review exact backend balances, corrections, dates, funding, warnings, and no-cash amount -> explicit confirmation -> execute`; the client does not recalculate financial values. Preview expiry, stale balance/draft versions, or a backend review-required blocker removes execution authority and requires a new preview. After execution, the original agreement is terminal `replaced` (shown in Done and All as **Closed — Replaced**, never Paid), both public loan records refresh, and the Loan Detail provides public source/replacement links. Reversal is a separate compensating command requiring its own non-blank reason, checkbox confirmation, and idempotency key; it remains subject to downstream safety checks.
