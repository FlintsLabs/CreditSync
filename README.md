...
Evidence declarations and accepted payment/payout evidence attempts are sticky safety requirements. CreditSync records the requirement before signing, download, DNS, or storage work, retains it when an attempt fails or is cleaned up, and requires every declared attachment to have an exact tenant-scoped finalized association before preview, posting, or activation. The Web disbursement editor keeps the selected draft and fields available after an evidence-blocked post, shows a localized safe blocker, and reuses the same idempotency key only when the operator manually retries. ChatGPT imports bind the tenant, target, idempotency key, and source-file fingerprint before network access; a changed descriptor under the same key stops for review without adding an attempt, while an exact retry resumes safely. When a legacy finalized association is first encountered, only its provable SHA-256 or source-fingerprint identity is seeded into the durable floor; known ChatGPT source aliases and direct content-hash aliases for the same association are not counted twice, and ambiguous identities are never inferred. The profile-bound read-only `workflow.resolve` endpoint reads only bounded authoritative target state and emits typed visible-tool guidance; it never authorizes a financial transition or invents future IDs, and if a host never reports an attachment, the backend cannot infer that file exists.

Cancelled duplicate reviews retain the ordinary strict evidence rule by default. Only an explicit, audited selection may use the canonical intake's single complete ready evidence for a cancelled, unposted duplicate that has exactly one persisted requirement, no effective evidence, and no authoritative attempts; selection is immutable and revalidated before replacement authorization. Exact participant identity decisions are available through `payment.identity-decision.preview` → confirmed `payment.identity-decision.execute` and the authenticated `/payment-intakes/:id/identity-decision/*` routes; previews show bounded amount, payer, and Bangkok business-time snapshots plus discrepancy seconds, and execution requires complete evidence for every connected participant unless that exact group has an existing audited canonical-coverage rule. Distinct decisions apply only to the reviewed pair, while same-payment groups are revalidated against current snapshots, ownership, evidence, and active postings. Mutable duplicate blockers enter this identity workflow before ordinary payment preview.

Payment recovery decisions are also explicit and append-only. Identity decisions snapshot the exact tenant-scoped participants and require a fresh preview, confirmation, reason, and idempotency key; effective same-payment components are transitive across valid decision snapshots, validated historical duplicate-review memberships, and replacement lineages, while superseded corrections are excluded and every active financial effect is rechecked before a posting exemption or group union. Identity-decision authorization also covers every connected member visible through those immutable relationships, including collector role boundaries. They do not rewrite posted or cancelled history and do not exempt a later third intake. A cancelled intake with failed or incomplete evidence follows `payment.evidence-recovery.preview` → confirmed `payment.evidence-recovery.execute` (or the matching authenticated REST routes) to create one linked recovery successor with a preserved requirement floor; any lower interpretation requires a confirmed, reasoned before/after requirement decision, and the successor remains blocked until its required evidence and ordinary payment preview are complete. A cancelled recovery successor can itself be recovered; evidence coverage follows only executed recovery receipts at every step and requires a complete final successor. When recovered groups are joined, each incomplete historical member must have its own authorized coverage; one successor need not cover unrelated recovery histories. Requesting a new identity preview clears the previous UI confirmation. Recovery is bounded by tenant-first deterministic identity locks and retries only confirmed PostgreSQL deadlock/serialization failures.
...
Unposted payment intakes can be explicitly cancelled from the Web inbox or MCP after inspecting the server-derived capability and confirming a reason/state hash. Cancellation retains the original evidence and audit history, excludes the intake from processing, and is not a refund or debt waiver. A cancelled intake may be replaced only through `payment.replacement.inspect` → explicit confirmation → `payment.replacement.create`; the replacement is a new draft with immutable evidence lineage and follows the ordinary preview/post workflow. Posted records continue through the existing compensating reversal workflow.
...
Release `v0.4.46` includes the independently reviewed workflow resolver and sticky evidence guards, deployed on 14 September 2026 after a verified PostgreSQL/MinIO backup and isolated restore rehearsal. Combined backend/cache, frontend, plugin, typecheck, protocol and production verification is recorded in [the acceptance matrix](docs/operations/workflow-resolver-acceptance.md). Existing ChatGPT connections must refresh/reconnect and verify tool visibility; server verification does not prove that a real mobile attachment reached the importer. Real-host/device and long-running canary acceptance remain explicit.
...
- group multiple Loan List cards under the canonical borrower name while retaining the same card layout and interactions
- view Loan Detail repayment and payment-history tables with row numbers, total-record counts, configurable page sizes, summary counts, and clear paid/due/overdue status indicators
...
- present responsive Loan List cards as collection-first summaries with grouped arrears, overdue age, contract terms, localized dates/currency, profile metadata, and actionable unassigned-agent state
...
- `POST /loan-renewals/preview` accepts `oldLoanPublicId`, `requestedPrincipal`, an explicit settlement policy, and reasoned `fee`, `penalty`, `other_charge`, or `waiver` lines. Scheduled daily renewals default to `full_contract_interest`; `accrued_to_date` must be selected explicitly.
- `POST /loan-renewals/:id/execute` requires the persisted `previewHash`, `confirmed: true`, a non-blank `reason`, and an `Idempotency-Key` header. A collection result also requires the explicit `confirmedCashDirection: "collection"` acknowledgment.
...
- reconcile approved historical `needs_review` allocations or restore a fully reversed evidence-backed source through `payment.restore.create` → `payment.restore.evidence.prepare` → signed PUT → `payment.restore.evidence.finalize` → `payment.restore.preview` → exact review → explicit confirmation → `payment.restore.execute`; an unwanted unposted restore draft follows `intake.get` → explicit confirmation → `payment.restore.cancel` → `intake.get` verification. Restore cancellation preserves the reversed source, restore evidence, and append-only history, creates no transaction/refund/balance change, and a cancelled child permits a later restore only with a new create key. Ordinary payment posting also requires the no-write preflight against the latest ready proposal. Restore returns source/child UUID lineage, copies the immutable original component split, and never copies bank-reference or evidence data
- replace an authorized cancelled, never-posted payment through `payment.replacement.inspect` → explicit confirmation → `payment.replacement.create`; this creates an audited draft only, retains the original amount/payer/receivedAt and references immutable finalized evidence. Re-inspect the current loan schedules, then use ordinary `payment.preview` → fresh ready/zero-variance confirmation → `payment.post`. This is separate from reversed-payment restore, cannot fork a lineage, and a cancelled successor may be replaced again.
- resolve an exact reviewed semantic duplicate through `payment.replacement.duplicate-review.preview` → explicit confirmation → `payment.replacement.duplicate-review.execute`. The tenant-scoped append-only review ledger authorizes only its exact cancelled candidates after rechecking cancellation provenance, amount, received timestamp, normalized payer, dependencies, hard identities, expiry, lineage, and ownership. It creates no payment or balance change; replacement and ordinary ready/zero-variance preview/post guards still run afterward. A reviewed duplicate may have no evidence only when the canonical cancelled source has complete finalized evidence; stale, ambiguous, conflicting, or newly introduced duplicates remain blocked.
- reject reconciliation, restore, allocation-correction, and temporal-reflow execution when an actually consumed payment intake has a sticky required attachment that is pending or rejected; a data-only intake with no known attachment attempt remains valid, and an already successful idempotent replay returns its original receipt before the guard
...
Loan detail pages include an Accrual Table tab for reviewing the authoritative materialized interest accrual ledger, including period dates, exact interest, paid and remaining amounts, and reversal status.
...
The funding matching workspace provides a searchable contract queue, exact live allocation and remaining-gap summaries, and a separate review step before any allocation is posted. The UI rejects amounts that exceed either the selected contract's remaining gap or the chosen funding source's remaining capacity; existing allocations remain append-only and corrections use the explicit reallocation workflow.
...
Current floating obligations resolve the complete reversal lineage before applying the requested business-date cutoff. A voided receipt therefore cannot reappear as paid interest or penalty merely because its reversal was processed later; the original receipts, reversals, and audit timestamps remain available in the append-only ledger. Read-only projections include missing accrual dates without persisting them; successful payment posting materializes those accruals through the receipt date in the same transaction.
...
Settlement and renewal previews and executions stop when the source loan has a known draft payout requirement that is pending or rejected. Restructure and temporal payment reflow likewise block a false-ready preview before creating replacement or reflow records, then recheck at execution. These alternate flows do not invent an attachment transport; unsupported evidence association remains human-review-only.
...
Active or paid daily-loan detail pages include the renewal control. The operator sees recovered principal, old outstanding principal, separate due interest/fee/penalty components, aggregate charges, waivers, settlement, net payout or collection, and the full replacement schedule before checking an explicit confirmation. Customer renewal summaries include a subtle finance watermark, a visually emphasized net-cash line, localized PNG download, and direct image copy when the browser supports the Clipboard API. Execution and reversal use separate stable retry keys and show explicit empty, unavailable, failed, or populated audit states with localized action labels.
...
The repository includes CreditSync Plugin `10.5.0` under [`plugins/creditsync`](./plugins/creditsync). It combines 11 orchestration skills with a private app reference to the HTTPS MCP endpoint and read-only `workflow.resolve` guidance; it does not bundle a local MCP process, URL, bearer token, OAuth, hooks, or plugin UI. Its payout evidence importer accepts the top-level ChatGPT file parameter only for an existing draft and leaves activation/posting to the separately confirmed workflow. Resolver guidance is deterministic and profile-bound: borrower targets are authorized by the backend, renewal previews use `oldLoanPublicId`, new-loan and payout-draft targets use their real closed-schema fields, ready existing payouts go to confirmation, scheduled close-out returns supported inspection guidance without floating settlement, existing loan targets receive inspection/activation prerequisites, and unsupported attachment transports route to human review. It never authorizes, confirms, calculates, persists a preview, or claims host/mobile adoption.
...
The infrastructure Compose file also includes the optional OpenAI Secure MCP Tunnel client as `tunnel-client`. It joins `creditsync_runtime` and forwards ChatGPT Developer Mode traffic to `http://backend:3000/mcp`. Set `TUNNEL_CLIENT_VERSION`, `CONTROL_PLANE_TUNNEL_ID`, `OPENAI_TUNNEL_API_KEY_FILE`, and `CREDITSYNC_MCP_AUTHORIZATION_FILE`, then start it with `--profile openai-mcp`; the two credential files are mounted as Compose secrets and must not be committed. The service uses the official `ghcr.io/openai/tunnel-client` image. See [`docs/operations/handoff-openai-secure-mcp-tunnel.md`](./docs/operations/handoff-openai-secure-mcp-tunnel.md) for setup and verification.
