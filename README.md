...
- reconcile approved historical `needs_review` allocations or restore a fully reversed evidence-backed source through `payment.reconcile.preview` / `payment.restore.preview` → no-write `payment.reconcile.preflight` → exact review → explicit confirmation → matching execute; ordinary payment posting also requires the no-write preflight against the latest ready proposal. Restore returns source/child UUID lineage, copies the immutable original component split, and never copies bank-reference or evidence data
