...
Reviewed against repository code on 2026-09-11. The clarifications below supersede ambiguous wording elsewhere in this document.

## Review decisions

- Cancellation changes workflow status and appends an immutable cancellation receipt/audit; the intake itself is not an append-only table. Preserve its original money, time, payer, evidence and financial lineage.
- Require `expectedStateHash` from the current authorized intake detail, in addition to reason and idempotency key. Hash intake status/updated state, latest proposal and batch membership/revision. Revalidate after locks; stale submissions fail before mutations. This is an implementation refinement of the approved stale-request requirement.
- Normalize reason by replacing control characters with spaces and trimming; require 1–2000 characters. Both REST and MCP require body fields `reason`, `idempotencyKey`, `expectedStateHash`; MCP also requires `paymentIntakePublicId`. Use the existing `PAYMENT_INTAKE_NOT_FOUND` error, including inaccessible targets.
- An immutable cancellation receipt stores the normalized request hash, original response, actor/source/request/correlation identifiers, audit UUID and timestamp. Unique `(tenant, operation, idempotencyKey)` prevents cross-target key reuse. Reauthorize before replay; same key and same normalized input returns the original receipt even though its state hash is now old. Changed input returns an idempotency conflict; a fresh key for an already-cancelled intake returns `PAYMENT_CANCEL_NOT_ALLOWED`.
- Return server-derived `cancellation` capability with `allowed`, `stateHash`, `blockedReason`, and accessible `batchPublicId`, plus safe cancellation metadata in detail/list responses. Viewer ownership alone must not confer write permissions; use existing mutation-capable owner/role policy.
- Lock borrower rows first in sorted order, then batch when applicable, then intake rows in sorted order. Revalidate membership and borrower resolution after locking. All participating membership writers must serialize with this order; a membership change requires fresh inspection rather than acquiring borrower locks out of order.
- Direct cancellation rejects every batch member with `PAYMENT_CANCEL_BATCH_REQUIRED`. Existing split followed by cancellation of the selected destination batch supports subset cancellation. Batch cancellation atomically cancels eligible unposted member intakes with receipts and audit, stales proposals, and clears confirmation. Reject the entire operation if a member has financial effects or incompatible lineage. Preserve `duplicate`/`reversed` terminal members. Legacy cancelled batches retain historical receipts; no silent migration of their children. An explicit maintenance command is outside this feature.
- Cancelled intakes cannot be reviewed, previewed, posted, edited, attached to another batch, or receive newly prepared/finalized evidence. Enforce at service and database boundaries, including late upload finalization. Preserve existing evidence for reading; do not delete objects or relax duplicate-reference/hash uniqueness.
- Reject linked restore/reconciliation/intermediary workflows with `PAYMENT_CANCEL_DEPENDENCY_REQUIRED` until their owning workflow offers an explicit supported cancellation. Do not strand reserved replacement links or orphan a remittance. A caller cannot bypass this by placing the intake in a batch.
- Cancellation does not mean a bank refund, debt waiver or proof that no money arrived. Confirmation copy states this and requires review of any real unallocated receipt. Reimporting the same evidence remains subject to duplicate checks; reopening/replacement of a cancelled intake is outside this release.
- Keep `duplicate` policy outside this change. Current SQL chronology and pure batch chronology differ on whether `duplicate` is terminal; add a regression documenting the difference, without claiming this feature fixes it. Likewise the earlier chat did not establish that the four older payer-name candidates caused the production conflict: no automatic data cleanup is authorized.
- Ordinary payment preview currently omits the posting chronology check. Report that as a separate limitation; cancellation removes the selected dependency but does not guarantee that another payment can post. Require fresh preview and dependency checks afterward.
- Posted records route to existing authorized reversal only; no automatic reversal, bulk cancellation, production cleanup, deployment or posting is part of implementation.
