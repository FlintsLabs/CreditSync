...
- Focused public regressions cover: never-mapped header behavior, map-A/preview/remap-B with later A allowed and later B blocked, explicit preview-B without staging mapping blocking later B, and public edit invalidating the old match proposal. Serialized DB verification: 46 passed, 0 failed, 145 expectations; solver: 6 passed, 0 failed, 17 expectations; typecheck passed. README was unchanged because the existing workflow description already documents mapping-aware invalidation and chronology. Full plan remains open.
