...
| --- | --- | --- | --- | --- |
| `intake.create`, `evidence.prepare`, ChatGPT payment import | payment intake | payment preview and payment posting kernel; batch and restore consumers recheck | direct signed PUT/finalize, supported ChatGPT import, batch staging | existing posted receipt is returned before new checks | adjacent payment tests |
| `payment.restore.preview`, `payment.restore.execute` | source payment intake and exact restore-draft intake | source assertion in reconciliation preview/execute and expected-count assertion in restore-draft inspection | existing restore evidence prepare/direct PUT/finalize only | existing restore receipt/replay branch is returned before the guard | `payment-reconciliation-service.test.ts` |
| `payment.batch.preview`, `payment.batch.execute` | every linked payment intake | preview readiness and locked execute path | existing intake/staging transports only | existing operation receipt is returned | adjacent batch tests |
| `payment.allocation-correction.execute` | consumed payment intake | locked correction context, before correction audit/group/transaction inserts | existing payment-intake evidence only | existing correction-group replay is returned before the guard | `payment-allocation-correction-service.test.ts` |
| `payment.reconciliation.reflow.execute` and temporal reflow | reconciliation source intake and each source transaction's payment intake, when present | locked reflow execution and per-source `executeTemporalReflow`, before audit/group/reversal/replacement writes | existing payment-intake evidence only; no new floating transport | existing reflow-group replay is returned before the guard | `payment-reconciliation-reflow-service.test.ts` |
| `loan.disbursement.draft`, `loan.disbursement.evidence.prepare`, ChatGPT payout import | loan-disbursement event | payout post under loan → event → evidence lock order; loan activation checks known draft payout requirements | direct signed PUT/finalize and supported ChatGPT import | existing posted payout receipt is returned | adjacent disbursement tests |
| `loan.activate` | known draft payout events for the loan | activation transaction before term/schedule effects | no implicit attachment transport | existing activation result is returned | adjacent activation tests |
| `loan.restructure.execute` | source loan's known draft payout events | source-loan evidence assertion after existing locks and before replacement loan/schedule/audit effects | no new restructure attachment transport; use existing payout workflow | existing restructure replay is returned before the guard | `loan-restructure-service.test.ts` |
| `loan.settlement.execute` | source loan's known draft payout events | source-loan evidence assertion after settlement replay and before close-account writes | no new settlement attachment transport | existing settlement replay is returned before the guard | `loan-settlement-service.test.ts` |
| `renewal.execute` | old/source loan's known draft payout events | source-loan evidence assertion after renewal replay and before close/replacement/payout effects | no new renewal attachment transport | existing renewal replay is returned before the guard | `loan-renewal-service.test.ts` |
| intermediary execute paths | service-specific records; no generic payment alias | existing service preview/execute checks; linked payment/payout targets must still pass their own guards | only the importer declared by that service | existing service receipt/idempotency behavior | human-review-only unsupported cases |
