...
4. Run application migrations once. Backend integration and financial tests must use a separately created synthetic disposable PostgreSQL database (the standard runner resets schemas); never point schema-resetting tests at the restored real-data database. On the restored database, run only read-only migration fingerprints, evidence-recovery checks, reconciliation queries, and MCP probes. Do not run `bun run --cwd backend test` or any fixture/reset suite against the restored real-data database; the disposable runner must create and own a separate synthetic database.
