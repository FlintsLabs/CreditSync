...
### Fixed
- Applied floating weekly rates through exact daily-prorated period snapshots anchored to the interest start date; made advance previews explicit about full-period interest, covered dates, true next boundary, net payout, and non-refundable treatment; kept the activation principal/rate basis immutable during corrections; calculated closing obligations from current unpaid balances, fees, and penalties without re-subtracting payment history; and enforced the frozen MCP repayment-type guard inside the activation row lock.
