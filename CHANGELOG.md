...
### Fixed
- Applied floating weekly rates once per seven-day period anchored to the interest start date across preview, activation, accrual, payment health, and append-only correction; stabilized malformed-policy errors, rejected contradictory draft terms, cleared inherited installment metadata only on repayment-type transitions, and stopped frozen MCP single-payment activation before mutation.
