...
### Fixed
- Applied floating weekly rates through exact daily-prorated period snapshots anchored to the interest start date, kept interim accrual out of normal payment allocation until the weekly boundary, preserved non-refundable first-period deductions and append-only corrections, stabilized malformed-policy errors without widening the REST enum, and enforced the frozen MCP repayment-type guard inside the activation row lock.
