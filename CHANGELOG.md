...
### Fixed
- Preserved independent inbound and outbound restructure lineage across multi-hop and reversed contract chains, while keeping each replacement loan's opening components and waivers anchored to its own inbound restructure; expanded REST regression coverage for role scope, cache invalidation, malformed identifiers and money, confirmation/idempotency failures, and conflicting retries.
- Preserved current net carried restructure components in replacement-loan rollups and early-settlement previews, unified general and new-contract interest waivers under one deterministic economic cap, retained general-waiver FIFO while assigning early new-interest waivers to the latest unearned future schedules with durable settlement-period provenance, refreshed waiver-adjusted economic status and due rollups without mutating contract schedules, routed external settlement credits and reversals through zero-sum payment and funding ledgers, ignored exactly reversed downstream payments when evaluating waiver reversibility, and strengthened settlement balance and concurrency guards.
- Applied floating weekly rates through exact daily-prorated period snapshots anchored to the interest start date; exposed the approved full-period, first-period, advance-interest, net-disbursement, covered-date, due/next-accrual, and non-refundable preview fields while preserving the frozen MCP adapter; replaced mutable penalty snapshots with an immutable dated due-group ledger, exact interest/penalty allocations, audited compensating adjustments and reversals, exact Bangkok legacy cutover, pure historical projections, and one health item per overdue weekly group; blocked backdated overlap with later exact allocations, assessed fixed fees when reversals first reopen overdue basis, and enforced same-loan provenance, append-only history, referenced-transaction immutability, exact transaction-component allocation totals, assessed penalty due groups, non-negative and paid penalty floors, and paid-cache equality at the PostgreSQL boundary; kept the activation principal/rate basis immutable during corrections; calculated closing and health obligations from current unpaid balances, fees, and penalties without global payment offsets; and enforced the frozen MCP repayment-type guard inside the activation row lock.
- Separated restructure creation, execution, and reversal actor sources so mixed-source reversals enforce the user actor belonging to each operation without permitting execution provenance to change.
- Prevented activated and terminal loans from returning to draft, while allowing system-origin restructure and waiver execution/reversal records to omit user actors only when complete audit and correlation context remains present.
