...
### Fixed
- Required literal confirmation in the intermediated-disbursement eval harness, validated retained-balance calls and every intermediary-flow input/output against the full frozen JSON schemas, verified signed-upload descriptors against unchanged fixture bytes and SHA-256, and enriched group inspection with normalized references plus safe per-event evidence status/count/public metadata while excluding checksums and storage details.
