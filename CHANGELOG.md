...
### Fixed
- Applied floating weekly rates through exact daily-prorated period snapshots anchored to the interest start date; exposed inclusive covered dates separately from the excluded due/next-accrual boundary and the explicit non-refundable policy; kept the activation principal/rate basis immutable during corrections; made overdue penalties durable per due group with penalty-first payment allocation and exact reversal; calculated closing and health obligations from current unpaid balances, fees, and penalties without global payment offsets; and enforced the frozen MCP repayment-type guard inside the activation row lock.
