...
- Added 14 closed-schema intermediary profile, masked-bank-account, assignment, managed-loan, and multi-leg disbursement MCP tools plus CreditSync Plugin `5.0.0` orchestration for exact three-slip evidence, zero-variance confirmation, stale-state stops, and compensating reversal.
...
### Fixed
- Corrected additive `0036` upgrade projection to derive each floating accrual's contractual period amount from its immutable principal/rate snapshots and to retain weekly versus daily units on existing rate periods.
