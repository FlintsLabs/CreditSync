...
### Fixed
- Completed durable restore-backfill receipt and historical explicit reconciliation concurrency coverage: concurrent same-key backfills return one exact receipt, no-op commands are audited once, tenant/FK/immutability boundaries are enforced, and non-restore execution waits for borrower locks before intake/loan locks and rejects a stale ordinary posting without compensation.
