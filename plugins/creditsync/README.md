...
- Before any payment reconciliation or posting confirmation, run `payment.reconcile.preflight`. It is explicitly no-write (`wouldWrite: false`) and must be `ready_to_execute` with complete provenance, current hash/version, and passing checks before an execute/post call.
