...
The intermediated-disbursement fixtures prove exact borrower and intermediary resolution, active assignment inspection, a three-leg group, unchanged-byte PUT/finalize for every supplied slip, group re-inspection, zero retained balance and variance, exact preview presentation, and explicit confirmation before post. Ambiguous identity, missing assignment/evidence/confirmation, duplicate transfer, amount/payee mismatch, unexplained retained balance, and stale preview all stop safely; stale state is inspected and re-previewed but requires fresh confirmation before any second post.
