...
The upload-first batch boundary also documents `payment.batch.staging.extract`: the tool is invoked only after staging evidence finalization, returns revision/checksum-bound candidate fields for human review, and is forbidden from creating an intake or financial record. Local OCR quality is not represented as a live eval result; only the disposable synthetic extraction fixture is verified in this checkout.
