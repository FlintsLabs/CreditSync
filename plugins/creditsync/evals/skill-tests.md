...
| `manage-disbursements` | actual payout with optional evidence, variance, post, and reversal | Used draft → evidence prepare → unchanged-byte PUT → finalize → explicit post, surfaced the backend variance without schedule mutation, and required a separate reversal reason/key. |
...
The disbursement lifecycle fixture proves the exact draft → evidence prepare → PUT → finalize → post → reverse ordering. The variance fixture stops before post and the schedule-mutation fixture proves that no loan preview, draft, or activation is attempted. Both post and reverse use different stable idempotency keys, while reversal carries a specific human-confirmed reason.
