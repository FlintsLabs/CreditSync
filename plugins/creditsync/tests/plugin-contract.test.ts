...
describe("CreditSync plugin 10.0.0 contract", () => {
    ...
    test("frozen full MCP metadata matches an actual MCP tools/list response", async () => {
        ...
        expect(contract.compatibility).toBe("Tool names, full input/output schemas, descriptions, annotations, and file-parameter metadata are frozen for plugin 10.0.0; breaking changes require plugin 11.0.0.");
        expect(contract.tools.map((tool) => tool.name)).toEqual([...MCP_TOOL_NAMES]);
        expect(contract.tools).toHaveLength(129);
    ...
    test("eval catalog covers every required positive and negative workflow", async () => {
        ...
        expect(catalog.cases?.filter((entry) => entry.kind === "positive")).toHaveLength(45);
        expect(catalog.cases?.filter((entry) => entry.kind === "negative")).toHaveLength(65);
    });
