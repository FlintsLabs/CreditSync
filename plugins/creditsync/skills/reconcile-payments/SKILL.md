...
To abandon an unposted restore draft, call `intake.get` and inspect its separate `restoreCancellation` capability and current state hash. After explicit human confirmation, call `payment.restore.cancel` with the exact `restoreDraftPublicId`, normalized reason, current hash, and a new stable idempotency key, then call `intake.get` again. This audited cancellation marks only the restore child `cancelled`; it preserves the reversed source, restore evidence, and append-only history, creates no transaction/refund/balance change, and is not proof that the original transfer did not occur. The ordinary `payment.cancel` tool must continue to reject restore children. A cancelled child remains historical and does not block a later restore attempt, but the original create key never revives it and only one non-cancelled child may exist at a time.

## Review payment identity explicitly

When two accessible payment intakes are ambiguous, use `payment.identity-decision.preview` with the complete participant UUID set and choose `same_payment` or `distinct_payment` only after showing immutable amount/time/reference snapshots. Obtain explicit confirmation, then call `payment.identity-decision.execute` with the unchanged preview hash, exact reason, and stable idempotency key. A distinct decision is exact-pair scope; a same-payment decision follows the complete transitive identity group and cannot authorize two active posted receipts. Re-inspect after any lifecycle, evidence, replacement, or dependency change. Stale/expired previews, hard reference conflicts, cross-tenant participants, changed ownership, active duplicate postings, and changed idempotency payloads stop without a write.

## Recover cancelled evidence

For an accessible cancelled, never-posted payment with failed, expired, partial, or missing uploads, call `payment.evidence-recovery.preview`, disclose the inherited requirement floor, reusable ready evidence UUIDs, source lineage, and any existing successor, then obtain explicit confirmation before `payment.evidence-recovery.execute`. Recovery creates one linked draft and preserves source evidence, attempts, requirements, cancellation, and audit history; it never uploads or posts by itself. Reuse only exact ready evidence references and complete missing slots on the child through the normal prepare/PUT/finalize flow. A lower expected count requires `requirementDecision` with `confirmed: true` and an audited before/after reason; never silently lower requirements. Replay the same command only with the same key and request, and revalidate tenant/owner access even on replay.

## Cancel an unwanted payment intake

For an unwanted `draft`, `needs_review`, or `ready` intake, call `intake.get` first and verify the current `cancellation.allowed`, state hash, status, ownership, and any batch/dependency warning. Obtain explicit human confirmation of the normalized reason, then call `payment.cancel` with the exact `paymentIntakePublicId`, current `expectedStateHash`, and one stable idempotency key. Reuse that same key and request only when retrying an uncertain response; a changed reason or stale hash requires a fresh inspection and renewed confirmation. Cancellation preserves the original intake, evidence, duplicate detection, and audit history; it is not a refund, debt waiver, or proof that no money arrived. A batch member must use the batch cancellation/split workflow, and a posted payment must use the existing reversal workflow. A `reversed` intake is terminal: do not send it to cancellation or attempt a second reversal. After cancellation, verify the detail/history and run a fresh payment preview: clearing one chronology blocker does not guarantee posting because ordinary preview has separate chronology/dependency limitations. Never infer cancellation from payer similarity or perform bulk cleanup.

For a cancelled, never-posted payment whose cancellation receipt and finalized evidence are intact, use the separate replacement workflow: call `payment.replacement.inspect`, show eligibility/blockers and the state hash, then after explicit confirmation call `payment.replacement.create` with a reason, current hash, and stable idempotency key. This creates only one audited `draft` successor, retains the original received timestamp and authoritative amount/payer/identity, and references immutable source evidence without copying, moving, exposing, or clearing hashes. It is not `payment.restore`, does not accept edited payment data, and does not post money. Re-inspect the current contract/schedule, call ordinary `payment.preview` with exact schedule UUID allocations, and post only a fresh ready/zero-variance/current proposal after separate confirmation. If the successor is cancelled, repeat replacement from that cancelled successor; never fork from an ancestor, replace a posted/reversed child, or bypass duplicate/tenant/role/evidence blockers.

If replacement inspection or creation reports `PAYMENT_DUPLICATE_REQUIRES_REVIEW`, do not pass the warning text as a reason and do not clear it. For the exact canonical cancelled source and an explicit bounded set of cancelled semantic-duplicate drafts, first call `payment.replacement.duplicate-review.preview`, verify exact amount, received timestamp, normalized payer, cancellation provenance, complete finalized canonical evidence, zero candidate dependencies, no hard-identity conflict, tenant/role scope, and expiry. After the operator confirms that exact preview, call `payment.replacement.duplicate-review.execute` with the unchanged preview hash and a new stable idempotency key. The review creates no payment or balance change; then re-run replacement inspect/create and ordinary payment preview/post. Any stale/expired preview, changed candidate, new duplicate, conflicting hard identity, lineage fork/cycle, or idempotency/ownership conflict stops without a financial write. A candidate without evidence may be reviewed only when the canonical source has complete finalized evidence.

For a confirmed cancelled duplicate that declares exactly one attachment but has never attempted an upload, preview may explicitly select its UUID in `canonicalEvidenceCandidatePublicIds`. Use this only when the canonical source has exactly one complete ready evidence item and the candidate has no direct evidence, authoritative attempt, or supplemental evidence in any status. Verify the echoed selection before confirming execution. This records an immutable review policy; it does not clear the candidate’s requirement or alter cancelled history. Omit the selection for ordinary reviews. Any changed evidence or requirement invalidates authorization and requires review.
