...
When an operator confirms that a posted payment was reversed by mistake, inspect the exact reversed intake and its source/child lineage, then call `payment.restore.preview` with a non-blank reason. The backend derives the original transaction component split; callers cannot change the amount, target loan, schedule, principal, interest, fee, or penalty values. Require finalized ready evidence, complete exact compensating reversals, and no existing child. Display the source snapshot and exact correction before calling `payment.restore.execute` with explicit confirmation, unchanged hashes, the same reason, and a new stable idempotency key. Execution creates one child intake with no bank reference or copied evidence, keeps the source `reversed`, and returns both source and posted child public IDs. Stale, missing-evidence, partial-reversal, duplicate-child, or conflicting-idempotency results stop for review.
