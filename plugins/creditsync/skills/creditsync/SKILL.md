...
Atomic payment batches use the closed resumable `payment.batch.stage` → `payment.batch.staging.evidence.prepare` → `payment.batch.staging.evidence.finalize` → `payment.batch.staging.review` workflow, followed by `payment.batch.preview`, `payment.batch.decision` when chronology acknowledgement is required, and `payment.batch.execute`. The lower-level `payment.batch.capture`, `payment.batch.create`, `payment.batch.item.add`, `payment.batch.evidence.prepare`, `payment.batch.evidence.finalize`, `payment.batch.evidence.prepare-many`, `payment.batch.evidence.finalize-many`, `payment.batch.get`, `payment.batch.workspace`, `payment.batch.staging.edit`, `payment.batch.split`, and `payment.batch.cancel` tools remain available. Inspect first, preview the complete batch, obtain explicit confirmation, execute once with stable idempotency, and verify every posted item; never continue a partial batch after ambiguity or duplicate review.
