...
3. Attach every supplied slip to its exact event. Retain the unchanged bytes' SHA-256, size, and MIME type; call `intermediary.disbursement.evidence.prepare` and retain its evidence/file public UUIDs plus current upload descriptor. Require prepare metadata to match the supplied bytes. PUT only those unchanged bytes with every returned header, then call `intermediary.disbursement.evidence.finalize` for the exact prepared evidence UUID and require the same evidence UUID, file UUID, MIME, size, and SHA-256 back. If prepare reports `ready`, do not PUT or finalize again, but still require its immutable IDs and metadata to match. Missing/expired upload data, a hash conflict, or any binding mismatch stops the workflow.
