...
4. Call `intermediary.disbursement.get` and compare all recorded amounts, roles, payees, references, and evidence states with the supplied transfers. General MCP inspection must never expose or request an evidence access URL through MCP; evidence viewing belongs to the REST/Web UI access flow.
