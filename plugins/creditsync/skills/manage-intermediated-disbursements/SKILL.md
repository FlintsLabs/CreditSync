...
4. Assignment changes are separate administrative writes. Use `intermediary.assignment.create` or `intermediary.assignment.end` only after an authorized human confirms the exact parties, role, effective time, and stable idempotency key. Save bank details through `intermediary.bank-account.save`; report only the returned masked account. Retain and report the audit public UUID and correlation UUID returned by each of these writes.
...
3. Attach every supplied slip to its exact event. Retain the unchanged bytes' SHA-256, size, and MIME type; call `intermediary.disbursement.evidence.prepare` and retain its evidence/file public UUIDs, audit public UUID, correlation UUID, and current upload descriptor. Require prepare metadata to match the supplied bytes. PUT only those unchanged bytes with every returned header, then call `intermediary.disbursement.evidence.finalize` for the exact prepared evidence UUID and require the same evidence UUID, file UUID, MIME, size, and SHA-256 back while retaining its own audit public UUID and correlation UUID. If prepare reports `ready`, do not PUT or finalize again, but still require its immutable IDs and metadata to match. Missing/expired upload data, a hash conflict, or any binding mismatch stops the workflow.
4. Call `intermediary.disbursement.get` and compare every event's amount, role, payee, normalized reference display, evidence status/count, intent UUID, file UUID, and MIME type with the exact supplied transfer and retained prepare result. General MCP inspection must never expose or request an evidence access URL through MCP, and must also exclude storage keys and checksums; evidence viewing belongs to the REST/Web UI access flow.
...
| Duplicate transfer, event/slip binding mismatch, or evidence failure | Stop before preview/post and inspect the existing state. |
