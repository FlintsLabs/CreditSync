...
export async function validatePlugin() {
    ...
    if (manifest.version !== "2.3.0") errors.push("manifest version must be 2.3.0");
    ...
    for (const entry of forbiddenEntries) {
        if (existsSync(resolve(pluginRoot, entry))) errors.push(`forbidden 2.3.0 capability exists: ${entry}`);
    }
...
if (import.meta.main) {
    ...
    console.log(`CreditSync plugin validation passed (2.3.0, 8 skills, 40 tools, no bundled MCP/secrets; private app: ${registration}${registration === "placeholder" ? ", non-live" : ""}).`);
}
