...
const expectedSkills = ["creditsync", "manage-borrowers", "reconcile-payments", "reconcile-intermediary-remittances", "manage-loans", "manage-floating-interest-rates", "manage-disbursements", "renew-daily-loan"];
...
export async function validatePlugin() {
    ...
    if (manifest.version !== "2.4.0") errors.push("manifest version must be 2.4.0");
    ...
    for (const entry of forbiddenEntries) {
        if (existsSync(resolve(pluginRoot, entry))) errors.push(`forbidden 2.4.0 capability exists: ${entry}`);
    }
...
if (import.meta.main) {
    ...
    console.log(`CreditSync plugin validation passed (2.4.0, 8 skills, 41 tools, no bundled MCP/secrets; private app: ${registration}${registration === "placeholder" ? ", non-live" : ""}).`);
}
