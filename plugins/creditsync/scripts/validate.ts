...
export async function validatePlugin() {
    ...
    if (manifest.version !== "10.6.0") errors.push("manifest version must be 10.6.0");
...
if (import.meta.main) {
    ...
    console.log(`CreditSync plugin validation passed (10.6.0, 11 skills, ${MCP_TOOL_NAMES.length} tools, no bundled MCP/secrets; private app: ${registration}${registration === "placeholder" ? ", non-live" : ""}).`);
}
