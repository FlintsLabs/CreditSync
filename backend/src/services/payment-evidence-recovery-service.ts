...
async function sourceState(ctx: CommandContext, source: typeof paymentIntakes.$inferSelect, tx: DbExecutor) {
    ...
    const readyIds = evidence.filter((item) => item.status === "ready" && item.finalizedAt !== null && item.fileId !== null).map((item) => item.sourceEvidenceId).sort((a, b) => a - b);
    const floor = Math.max(requirement?.expectedCount ?? 0, evidence.length, attempts, source.evidenceRequired ? 1 : 0);
    const stateHash = digest({ status: source.status, updatedAt: source.updatedAt.toISOString(), cancelledAt: source.cancelledAt?.toISOString() ?? null, requirement: requirement?.publicId ?? null, expectedCount: requirement?.expectedCount ?? 0, attempts, evidence: evidence.map((item) => ({ id: item.sourceEvidenceId, sourceIntakeId: item.sourceIntakeId, source: item.source, status: item.status, finalizedAt: item.finalizedAt?.toISOString() ?? null, fileId: item.fileId })).sort((a, b) => a.id - b.id) });
    return { evidence, readyIds, floor, stateHash };
}

export type PaymentEvidenceRecoveryPreviewInput = { sourcePaymentIntakePublicId: string; reason: string; expectedCount: number; reuseEvidence: boolean; idempotencyKey: string; requirementDecision?: { confirmed: true; reason: string } };
...
export async function previewPaymentEvidenceRecovery(ctx: CommandContext, input: PaymentEvidenceRecoveryPreviewInput, executor?: DbExecutor) {
    const run = async (tx: DbExecutor) => {
        ...
        const reinterpret = input.expectedCount < state.floor ? input.requirementDecision : undefined;
        if (input.expectedCount < state.floor && (!reinterpret?.confirmed || !reinterpret.reason?.trim())) throw new DomainError("PAYMENT_EVIDENCE_RECOVERY_REQUIREMENT_FLOOR", "Reducing an inherited evidence requirement requires an explicit confirmed reason", 409, { requirementFloor: state.floor });
        const requestHash = digest({ sourcePaymentIntakePublicId: source.publicId, reason: input.reason.trim(), expectedCount: input.expectedCount, reuseEvidence: input.reuseEvidence === true, requirementDecision: reinterpret ?? null, idempotencyKey: input.idempotencyKey.trim() });
...
export async function executePaymentEvidenceRecovery(ctx: CommandContext, input: { recoveryPreviewPublicId: string; previewHash: string; confirmed: true; reason: string; idempotencyKey: string }, executor?: DbExecutor) {
    const run = async (tx: DbExecutor) => {
        ...
        await lockPaymentWorkflowIdentity(ctx, tx, [preview.sourcePaymentIntakeId.toString()]);
        const accessSource = await tx.query.paymentIntakes.findFirst({ where: and(eq(paymentIntakes.tenantId, ctx.tenantId), eq(paymentIntakes.id, preview.sourcePaymentIntakeId)) });
        if (!accessSource || (!canAccessTenantWideData({ role: actor.role ?? "viewer" }) && accessSource.ownerUserId !== actor.id)) throw new DomainError("PAYMENT_INTAKE_NOT_FOUND", "Payment intake not found", 404);
        ...
        await lockPaymentWorkflowIdentity(ctx, tx, [source.publicId]);
        const state = await sourceState(ctx, source, tx);
        if (state.stateHash !== preview.sourceStateHash || (preview.expectedCount < state.floor && !preview.requirementDecisionReason)) throw new DomainError("PAYMENT_EVIDENCE_RECOVERY_PREVIEW_STALE", "Source evidence state changed; preview again", 409);
        ...
        const audit = await createAuditLog(tx, { tenantId: ctx.tenantId, actorUserId: actor.id, actorSource: ctx.actorSource, requestId: ctx.requestId, correlationId: ctx.correlationId, entityType: "payment_intake", entityId: source.publicId, action: "evidence_recovery_draft_created", payload: { sourcePaymentIntakePublicId: source.publicId, expectedCountBefore: preview.requirementFloor, expectedCountAfter: preview.expectedCount, requirementDecisionReason: preview.requirementDecisionReason, expectedCount: preview.expectedCount, requirementFloor: preview.requirementFloor, reusedEvidenceCount: preview.reusableEvidenceIds.length, previewPublicId: preview.publicId, requestHash } });
        ...
        if (preview.reuseEvidence && preview.reusableEvidenceIds.length) {
            const reusable = state.evidence.filter((item) => preview.reusableEvidenceIds.includes(item.sourceEvidenceId) && item.status === "ready" && item.finalizedAt !== null && item.fileId !== null);
            if (reusable.length !== preview.reusableEvidenceIds.length) throw new DomainError("PAYMENT_EVIDENCE_RECOVERY_PREVIEW_STALE", "A referenced evidence item changed; preview again", 409);
            await tx.insert(paymentReplacementEvidenceReferences).values(reusable.map((item) => ({
                tenantId: ctx.tenantId, lineageId: lineage.id, replacementPaymentIntakeId: child.id,
                sourcePaymentIntakeId: item.sourceIntakeId,
                sourceEvidenceId: item.sourceEvidenceId > 0 ? item.sourceEvidenceId : null,
                sourceSupplementId: item.sourceEvidenceId < 0 ? -item.sourceEvidenceId : null,
            })));
        }
