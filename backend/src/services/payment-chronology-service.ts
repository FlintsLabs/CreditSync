    ...
    const rows = await tx.execute(sql`SELECT DISTINCT l.borrower_id FROM loans l WHERE l.tenant_id = ${tenantId} AND (
        ...
        OR l.id IN (SELECT a.loan_id FROM payment_batch_items bi
            ...
            WHERE bi.tenant_id = ${tenantId} AND bi.payment_intake_id = ${intakeId} AND b.status <> 'cancelled'
              AND a.preview_id = (SELECT p.id FROM payment_batch_previews p WHERE p.tenant_id = bi.tenant_id AND p.batch_id = bi.batch_id AND p.status = 'ready' AND p.id = (SELECT latest.id FROM payment_batch_previews latest WHERE latest.tenant_id = p.tenant_id AND latest.batch_id = p.batch_id ORDER BY latest.version DESC LIMIT 1)))
