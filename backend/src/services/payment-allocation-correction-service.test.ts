...
import { db } from "../db";
import { borrowers, loanSchedules, loans, paymentAllocationCorrectionEntries, paymentAllocationCorrectionGroups, paymentIntakes, transactions, users } from "../db/schema";
...
async function fixture() {
    ...
    const schedules = await db.insert(loanSchedules).values([
        ...
        { tenantId, loanId: loan.id, installmentNo: 2, dueDate: "2026-09-07", scheduledPrincipal: "173.92", scheduledInterest: "26.08", scheduledFee: "0.00", scheduledTotal: "200.00", paidTotal: "200.00", paidPenalty: "0.00", remainingDue: "0.00", status: "paid" },
    ]).returning();
...
describe("scheduled payment allocation correction", () => {
    ...
    integrationTest("executes append-only compensation/replacement and supports idempotent replay", async () => {
        ...
        expect(history[1]).toMatchObject({ entryType: "reversal", amount: "-200.00", principalComponent: "-173.92", interestComponent: "-26.08", reversedTransactionId: seeded.source.id, scheduleId: seeded.schedules[1]!.id, transactionDate: seeded.source.transactionDate });
