...
async function fixture(targetScheduledTotal = "200.00") {
    ...
    const schedules = await db.insert(loanSchedules).values([
        ...
        { tenantId, loanId: loan.id, installmentNo: 2, dueDate: "2026-09-07", scheduledPrincipal: "173.92", scheduledInterest: "26.08", scheduledFee: "0.00", scheduledTotal: targetScheduledTotal, paidTotal: "200.00", paidPenalty: "0.00", remainingDue: targetScheduledTotal === "200.00" ? "0.00" : "100.00", status: targetScheduledTotal === "200.00" ? "paid" : "pending" },
    ]).returning();
