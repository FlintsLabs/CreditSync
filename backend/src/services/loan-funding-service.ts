...
import { db } from "../db";
import { bankLoans, bankProfiles, loanFundingAllocations, loans, users } from "../db/schema";
...
function amount(value: string) { try { const d = new FinancialDecimal(value); if (!d.isFinite() || d.lte(0)) throw new Error(); return d.toFixed(2); } catch { throw new DomainError("INVALID_MONEY", "allocatedAmount must be a positive decimal", 400); } }
...
async function resolve(tx: any, ctx: CommandContext, input: FundingAllocationInput, lock = true) {
    ...
    const ids = [loan.id, ...(drawdown ? [drawdown.id] : []), ...(profile ? [profile.id] : [])].sort((a, b) => a - b);
    if (lock) await tx.execute(sql`SELECT id FROM loans WHERE tenant_id = ${ctx.tenantId} AND id = ${loan.id} FOR UPDATE`);
    if (drawdown) await tx.execute(sql`SELECT id FROM bank_loans WHERE tenant_id = ${ctx.tenantId} AND id = ${drawdown.id} FOR UPDATE`);
    return { loan, profile, drawdown, ids };
}
