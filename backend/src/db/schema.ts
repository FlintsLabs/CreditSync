...
export const paymentEvidenceRecoveryPreviews = pgTable("payment_evidence_recovery_previews", {
    id: serial("id").primaryKey(), publicId: uuid("public_id").default(sql`uuidv7()`).notNull().unique(), tenantId: tenantId,
    sourcePaymentIntakeId: integer("source_payment_intake_id").notNull(), reason: text("reason").notNull(), expectedCount: integer("expected_count").notNull(), requirementFloor: integer("requirement_floor").notNull(), sourceStateHash: text("source_state_hash").notNull(), reusableEvidenceIds: jsonb("reusable_evidence_ids").$type<number[]>().notNull(), reuseEvidence: boolean("reuse_evidence").notNull(), previewHash: text("preview_hash").notNull(), requestHash: text("request_hash").notNull(), auditPublicId: uuid("audit_public_id").notNull(), expiresAt: timestamp("expires_at", { withTimezone: true }).notNull(), requestId: text("request_id").notNull(), correlationId: text("correlation_id").notNull(), idempotencyKey: text("idempotency_key").notNull(), createdByUserId: integer("created_by_user_id").notNull(), createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
...
export const paymentBatches = pgTable("payment_batches", {
    ...
    uniqueIndex("payment_batches_tenant_id_id_unique").on(table.tenantId, table.id),
    uniqueIndex("payment_batches_tenant_idempotency_unique").on(table.tenantId, table.createIdempotencyKey),
    check("payment_batches_status_check", sql`${table.status} IN ('draft', 'needs_review', 'ready', 'confirmed', 'posted', 'stale', 'cancelled')`),
    check("payment_batches_version_check", sql`${table.version} >= 0`),
    check("payment_batches_command_keys_check", sql`${table.createIdempotencyKey} ~ '[^[:space:]]' AND (${table.executeIdempotencyKey} IS NULL OR ${table.executeIdempotencyKey} ~ '[^[:space:]]')`),
    check("payment_batches_cancel_lifecycle_check", sql`(${table.status} = 'cancelled' AND ${table.cancelIdempotencyKey} IS NOT NULL AND ${table.cancelRequestHash} IS NOT NULL AND ${table.cancelReason} IS NOT NULL AND length(trim(${table.cancelReason})) > 0 AND ${table.cancelRevision} IS NOT NULL) OR (${table.status} <> 'cancelled' AND ${table.cancelIdempotencyKey} IS NULL AND ${table.cancelRequestHash} IS NULL AND ${table.cancelReason} IS NULL AND ${table.cancelRevision} IS NULL)`),
    check("payment_batches_posted_lifecycle_check", sql`(${table.status} = 'posted' AND ${table.postedAt} IS NOT NULL AND ${table.executeIdempotencyKey} IS NOT NULL) OR (${table.status} <> 'posted' AND ${table.postedAt} IS NULL)`),
    foreignKey({ name: "payment_batches_tenant_borrower_fk", columns: [table.tenantId, table.borrowerId], foreignColumns: [borrowers.tenantId, borrowers.id] }),
    foreignKey({ name: "payment_batches_tenant_created_by_fk", columns: [table.tenantId, table.createdByUserId], foreignColumns: [users.tenantId, users.id] }),
    foreignKey({ name: "payment_batches_tenant_updated_by_fk", columns: [table.tenantId, table.updatedByUserId], foreignColumns: [users.tenantId, users.id] }),
]);

export const paymentBatchStagingItems = pgTable("payment_batch_staging_items", {
    id: serial("id").primaryKey(),
    publicId: uuid("public_id").default(sql`uuidv7()`).notNull().unique(),
    tenantId: tenantId,
    batchId: integer("batch_id").notNull(),
    clientItemKey: text("client_item_key").notNull(),
    ...
    paymentIntakeId: integer("payment_intake_id"),
    batchItemId: integer("batch_item_id"),
    ...
    uniqueIndex("payment_batch_staging_tenant_id_id_unique").on(table.tenantId, table.id),
    uniqueIndex("payment_batch_staging_tenant_batch_client_key_unique").on(table.tenantId, table.batchId, table.clientItemKey),
    uniqueIndex("payment_batch_staging_tenant_batch_operation_unique").on(table.tenantId, table.batchId, table.operationKey).where(sql`${table.operationKey} IS NOT NULL`),
    check("payment_batch_staging_status_check", sql`${table.status} IN ('staged', 'needs_review', 'validated', 'failed')`),
    foreignKey({ name: "payment_batch_staging_tenant_batch_fk", columns: [table.tenantId, table.batchId], foreignColumns: [paymentBatches.tenantId, paymentBatches.id] }),
...
export const paymentBatchStagingEvidence = pgTable("payment_batch_staging_evidence", {
    ...
    publicId: uuid("public_id").default(sql`uuidv7()`).notNull().unique(),
    tenantId: tenantId,
    stagingItemId: integer("staging_item_id").notNull(),
    fileId: integer("file_id").notNull(),
    evidenceHash: text("evidence_hash").notNull(),
    mimeType: text("mime_type").notNull(),
    declaredSize: integer("declared_size").notNull(),
    ...
    uniqueIndex("payment_batch_staging_evidence_tenant_id_id_unique").on(table.tenantId, table.id),
    uniqueIndex("payment_batch_staging_evidence_tenant_hash_unique").on(table.tenantId, table.evidenceHash),
    uniqueIndex("payment_batch_staging_evidence_item_unique").on(table.tenantId, table.stagingItemId),
    check("payment_batch_staging_evidence_status_check", sql`${table.status} IN ('pending', 'ready', 'rejected')`),
    foreignKey({ name: "payment_batch_staging_evidence_tenant_item_fk", columns: [table.tenantId, table.stagingItemId], foreignColumns: [paymentBatchStagingItems.tenantId, paymentBatchStagingItems.id] }),
    foreignKey({ name: "payment_batch_staging_evidence_tenant_file_fk", columns: [table.tenantId, table.fileId], foreignColumns: [files.tenantId, files.id] }),
    foreignKey({ name: "payment_batch_staging_evidence_tenant_created_by_fk", columns: [table.tenantId, table.createdByUserId], foreignColumns: [users.tenantId, users.id] }),
    foreignKey({ name: "payment_batch_staging_evidence_tenant_updated_by_fk", columns: [table.tenantId, table.updatedByUserId], foreignColumns: [users.tenantId, users.id] }),
]);

export const paymentBatchOperationReceipts = pgTable("payment_batch_operation_receipts", {
    id: serial("id").primaryKey(),
    publicId: uuid("public_id").default(sql`uuidv7()`).notNull().unique(),
    tenantId: tenantId,
    batchId: integer("batch_id").notNull(),
    stagingItemId: integer("staging_item_id"),
    ...
    uniqueIndex("payment_batch_operation_receipts_tenant_id_id_unique").on(table.tenantId, table.id),
    uniqueIndex("payment_batch_operation_receipts_tenant_operation_unique").on(table.tenantId, table.operationType, table.operationKey),
    foreignKey({ name: "payment_batch_operation_receipts_tenant_batch_fk", columns: [table.tenantId, table.batchId], foreignColumns: [paymentBatches.tenantId, paymentBatches.id] }),
    foreignKey({ name: "payment_batch_operation_receipts_tenant_staging_fk", columns: [table.tenantId, table.stagingItemId], foreignColumns: [paymentBatchStagingItems.tenantId, paymentBatchStagingItems.id] }),
...
export const paymentBatchDecisions = pgTable("payment_batch_decisions", {
    ...
    uniqueIndex("payment_batch_decisions_tenant_id_id_unique").on(table.tenantId, table.id),
    check("payment_batch_decisions_action_check", sql`${table.action} = 'confirm_no_older_pending'`),
    check("payment_batch_decisions_reason_range_check", sql`length(trim(${table.reason})) > 0 AND ${table.fromDate} <= ${table.toDate}`),
    foreignKey({ name: "payment_batch_decisions_tenant_batch_fk", columns: [table.tenantId, table.batchId], foreignColumns: [paymentBatches.tenantId, paymentBatches.id] }),
    foreignKey({ name: "payment_batch_decisions_tenant_preview_fk", columns: [table.tenantId, table.previewId], foreignColumns: [paymentBatchPreviews.tenantId, paymentBatchPreviews.id] }),
    foreignKey({ name: "payment_batch_decisions_tenant_actor_fk", columns: [table.tenantId, table.createdByUserId], foreignColumns: [users.tenantId, users.id] }),
]);
...
export const paymentBatchItems = pgTable("payment_batch_items", {
    ...
    uniqueIndex("payment_batch_items_tenant_id_id_unique").on(table.tenantId, table.id),
    uniqueIndex("payment_batch_items_tenant_intake_unique").on(table.tenantId, table.paymentIntakeId),
    uniqueIndex("payment_batch_items_tenant_batch_order_unique").on(table.tenantId, table.batchId, table.itemOrder),
    check("payment_batch_items_order_check", sql`${table.itemOrder} > 0`),
    foreignKey({ name: "payment_batch_items_tenant_batch_fk", columns: [table.tenantId, table.batchId], foreignColumns: [paymentBatches.tenantId, paymentBatches.id] }),
    foreignKey({ name: "payment_batch_items_tenant_intake_fk", columns: [table.tenantId, table.paymentIntakeId], foreignColumns: [paymentIntakes.tenantId, paymentIntakes.id] }),
    foreignKey({ name: "payment_batch_items_tenant_staging_fk", columns: [table.tenantId, table.stagingItemId], foreignColumns: [paymentBatchStagingItems.tenantId, paymentBatchStagingItems.id] }),
]);

export const paymentBatchPreviews = pgTable("payment_batch_previews", {
    id: serial("id").primaryKey(),
    publicId: uuid("public_id").default(sql`uuidv7()`).notNull().unique(),
    tenantId: tenantId,
    batchId: integer("batch_id").notNull(),
    ...
    allocationSnapshotHash: text("allocation_snapshot_hash"),
    postingSequence: jsonb("posting_sequence").$type<string[]>(),
    ...
    uniqueIndex("payment_batch_previews_tenant_id_id_unique").on(table.tenantId, table.id),
    uniqueIndex("payment_batch_previews_tenant_batch_version_unique").on(table.tenantId, table.batchId, table.version),
    check("payment_batch_previews_version_check", sql`${table.version} > 0`),
    check("payment_batch_previews_status_check", sql`${table.status} IN ('needs_review', 'ready', 'stale', 'posted')`),
    foreignKey({ name: "payment_batch_previews_tenant_batch_fk", columns: [table.tenantId, table.batchId], foreignColumns: [paymentBatches.tenantId, paymentBatches.id] }),
    foreignKey({ name: "payment_batch_previews_tenant_created_by_fk", columns: [table.tenantId, table.createdByUserId], foreignColumns: [users.tenantId, users.id] }),
]);

export const paymentBatchAllocations = pgTable("payment_batch_allocations", {
    id: serial("id").primaryKey(),
    publicId: uuid("public_id").default(sql`uuidv7()`).notNull().unique(),
    tenantId: tenantId,
    previewId: integer("preview_id").notNull(),
    ...
    loanId: integer("loan_id").notNull(),
    scheduleId: integer("schedule_id"),
    targetKind: text("target_kind").default("scheduled").notNull(),
    floatingPlan: jsonb("floating_plan").$type<{ throughDate: string; allocations: Array<{ effectiveDate: string; dueDate: string; accrualDate: string | null; component: "interest" | "penalty"; amount: string }>; components: Record<string, string> }>(),
    amount: numeric("amount").notNull(),
    targetDueDate: date("target_due_date").notNull(),
    intent: text("intent").notNull(),
    calculatedComponents: jsonb("calculated_components").$type<Record<string, string>>().notNull(),
    status: text("status").default("proposed").notNull(),
}, (table) => [
    uniqueIndex("payment_batch_allocations_tenant_id_id_unique").on(table.tenantId, table.id),
    uniqueIndex("payment_batch_allocations_tenant_preview_order_unique").on(table.tenantId, table.previewId, table.allocationOrder),
    check("payment_batch_allocations_status_check", sql`${table.status} IN ('proposed', 'posted')`),
    check("payment_batch_allocations_target_check", sql`((${table.targetKind} = 'scheduled' AND ${table.scheduleId} IS NOT NULL AND ${table.floatingPlan} IS NULL) OR (${table.targetKind} = 'floating' AND ${table.scheduleId} IS NULL AND ${table.floatingPlan} IS NOT NULL AND jsonb_typeof(${table.floatingPlan}) = 'object' AND ${table.floatingPlan} ? 'throughDate' AND jsonb_typeof(${table.floatingPlan}->'allocations') = 'array')) IS TRUE`),
    check("payment_batch_allocations_intent_check", sql`${table.intent} IN ('on_time', 'advance', 'backdated')`),
    check("payment_batch_allocations_amount_check", sql`${table.amount} >= 0 AND scale(${table.amount}) <= 2 AND ${table.amount} NOT IN ('NaN'::numeric, 'Infinity'::numeric, '-Infinity'::numeric)`),
    foreignKey({ name: "payment_batch_allocations_tenant_preview_fk", columns: [table.tenantId, table.previewId], foreignColumns: [paymentBatchPreviews.tenantId, paymentBatchPreviews.id] }),
    foreignKey({ name: "payment_batch_allocations_tenant_item_fk", columns: [table.tenantId, table.itemId], foreignColumns: [paymentBatchItems.tenantId, paymentBatchItems.id] }),
    foreignKey({ name: "payment_batch_allocations_tenant_borrower_fk", columns: [table.tenantId, table.borrowerId], foreignColumns: [borrowers.tenantId, borrowers.id] }),
    foreignKey({ name: "payment_batch_allocations_tenant_loan_fk", columns: [table.tenantId, table.loanId], foreignColumns: [loans.tenantId, loans.id] }),
    foreignKey({ name: "payment_batch_allocations_tenant_schedule_fk", columns: [table.tenantId, table.scheduleId], foreignColumns: [loanSchedules.tenantId, loanSchedules.id] }),
]);
...
export const paymentAllocationCorrectionPreviews = pgTable("payment_allocation_correction_previews", {
    ...
    sourceScheduleId: integer("source_schedule_id").notNull(),
    targetScheduleId: integer("target_schedule_id").notNull(),
    loanId: integer("loan_id").notNull(),
    status: text("status").default("ready").notNull(),
    amount: numeric("amount").notNull(),
    principalComponent: numeric("principal_component").default("0").notNull(),
    ...
    uniqueIndex("payment_allocation_correction_previews_tenant_id_id_unique").on(table.tenantId, table.id),
    index("payment_allocation_correction_previews_tenant_source_idx").on(table.tenantId, table.sourceTransactionId, table.createdAt),
    check("payment_allocation_correction_previews_status_check", sql`${table.status} IN ('ready', 'blocked', 'executed', 'expired')`),
    check("payment_allocation_correction_previews_reason_check", sql`length(btrim(${table.reason})) > 0`),
    check("payment_allocation_correction_previews_amount_check", sql`${table.amount} > 0 AND scale(${table.amount}) <= 2 AND ${table.amount} = ${table.principalComponent} + ${table.interestComponent} + ${table.feeComponent} + ${table.penaltyComponent}`),
    foreignKey({ name: "payment_allocation_correction_previews_tenant_intake_fk", columns: [table.tenantId, table.paymentIntakeId], foreignColumns: [paymentIntakes.tenantId, paymentIntakes.id] }),
    foreignKey({ name: "payment_allocation_correction_previews_tenant_source_tx_fk", columns: [table.tenantId, table.sourceTransactionId], foreignColumns: [transactions.tenantId, transactions.id] }),
    foreignKey({ name: "payment_allocation_correction_previews_tenant_source_schedule_fk", columns: [table.tenantId, table.sourceScheduleId], foreignColumns: [loanSchedules.tenantId, loanSchedules.id] }),
    foreignKey({ name: "payment_allocation_correction_previews_tenant_target_schedule_fk", columns: [table.tenantId, table.targetScheduleId], foreignColumns: [loanSchedules.tenantId, loanSchedules.id] }),
    foreignKey({ name: "payment_allocation_correction_previews_tenant_loan_fk", columns: [table.tenantId, table.loanId], foreignColumns: [loans.tenantId, loans.id] }),
    foreignKey({ name: "payment_allocation_correction_previews_tenant_created_by_fk", columns: [table.tenantId, table.createdByUserId], foreignColumns: [users.tenantId, users.id] }),
    foreignKey({ name: "payment_allocation_correction_previews_tenant_executed_by_fk", columns: [table.tenantId, table.executedByUserId], foreignColumns: [users.tenantId, users.id] }),
]);

export const paymentAllocationCorrectionGroups = pgTable("payment_allocation_correction_groups", {
    id: serial("id").primaryKey(),
    publicId: uuid("public_id").default(sql`uuidv7()`).notNull().unique(),
    tenantId: tenantId,
    previewId: integer("preview_id").notNull(),
    ...
    foreignKey({ name: "payment_allocation_correction_groups_tenant_source_schedule_fk", columns: [table.tenantId, table.sourceScheduleId], foreignColumns: [loanSchedules.tenantId, loanSchedules.id] }),
    foreignKey({ name: "payment_allocation_correction_groups_tenant_target_schedule_fk", columns: [table.tenantId, table.targetScheduleId], foreignColumns: [loanSchedules.tenantId, loanSchedules.id] }),
    foreignKey({ name: "payment_allocation_correction_groups_tenant_loan_fk", columns: [table.tenantId, table.loanId], foreignColumns: [loans.tenantId, loans.id] }),
    foreignKey({ name: "payment_allocation_correction_groups_tenant_audit_fk", columns: [table.tenantId, table.auditPublicId], foreignColumns: [auditLogs.tenantId, auditLogs.publicId] }),
    foreignKey({ name: "payment_allocation_correction_groups_tenant_created_by_fk", columns: [table.tenantId, table.createdByUserId], foreignColumns: [users.tenantId, users.id] }),
]);

export const paymentAllocationCorrectionEntries = pgTable("payment_allocation_correction_entries", {
    id: serial("id").primaryKey(),
    publicId: uuid("public_id").default(sql`uuidv7()`).notNull().unique(),
    tenantId: tenantId,
    groupId: integer("group_id").notNull(),
    ...
    amount: numeric("amount").notNull(),
    principalComponent: numeric("principal_component").notNull(),
    interestComponent: numeric("interest_component").notNull(),
    feeComponent: numeric("fee_component").notNull(),
    penaltyComponent: numeric("penalty_component").notNull(),
    reason: text("reason").notNull(),
    auditPublicId: uuid("audit_public_id").notNull(),
    createdByUserId: integer("created_by_user_id"),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    ...
    foreignKey({ name: "payment_allocation_correction_entries_tenant_loan_fk", columns: [table.tenantId, table.loanId], foreignColumns: [loans.tenantId, loans.id] }),
    foreignKey({ name: "payment_allocation_correction_entries_tenant_schedule_fk", columns: [table.tenantId, table.scheduleId], foreignColumns: [loanSchedules.tenantId, loanSchedules.id] }),
    foreignKey({ name: "payment_allocation_correction_entries_tenant_audit_fk", columns: [table.tenantId, table.auditPublicId], foreignColumns: [auditLogs.tenantId, auditLogs.publicId] }),
    foreignKey({ name: "payment_allocation_correction_entries_tenant_created_by_fk", columns: [table.tenantId, table.createdByUserId], foreignColumns: [users.tenantId, users.id] }),
]);
...
export const intermediaryBankAccounts = pgTable("intermediary_bank_accounts", {
    ...
    check("intermediary_bank_accounts_identity_check", sql`${table.bankName} ~ '[^[:space:]]' AND ${table.accountName} ~ '[^[:space:]]' AND ${table.accountNumberHash} ~ '[^[:space:]]'`),
...
export const loanIntermediaryAssignments = pgTable("loan_intermediary_assignments", {
    ...
    check("loan_intermediary_assignments_idempotency_key_check", sql`${table.idempotencyKey} ~ '[^[:space:]]'`),
...
export const intermediatedDisbursementGroups = pgTable("intermediated_disbursement_groups", {
    ...
    check("intermediated_disbursement_groups_command_keys_check", sql`
        ${table.idempotencyKey} ~ '[^[:space:]]'
        AND (${table.postIdempotencyKey} IS NULL OR ${table.postIdempotencyKey} ~ '[^[:space:]]')
        AND (${table.reversalIdempotencyKey} IS NULL OR ${table.reversalIdempotencyKey} ~ '[^[:space:]]')
        AND (${table.reversalRequestHash} IS NULL OR ${table.reversalRequestHash} ~ '[^[:space:]]')
    `),
    ...
    check("intermediated_disbursement_groups_lifecycle_check", sql`
        ...
        OR (${table.status} = 'reversed'
            ...
            AND ${table.reversalRequestHash} IS NOT NULL AND ${table.reversalRequestHash} ~ '[^[:space:]]'
            AND ${table.reversalReason} IS NOT NULL AND ${table.reversalReason} ~ '[^[:space:]]')
    `),
...
export const intermediatedTransferEvents = pgTable("intermediated_transfer_events", {
    ...
    check("intermediated_transfer_events_command_keys_check", sql`
        ${table.idempotencyKey} ~ '[^[:space:]]'
        AND (${table.bankReferenceHash} IS NULL OR ${table.bankReferenceHash} ~ '[^[:space:]]')
    `),
...
export const intermediatedTransferEvidenceIntents = pgTable("intermediated_transfer_evidence_intents", {
    ...
    check("intermediated_transfer_evidence_intents_metadata_check", sql`${table.evidenceHash} ~ '[^[:space:]]' AND ${table.mimeType} ~ '[^[:space:]]' AND ${table.declaredSize} > 0`),
...
export const intermediatedDisbursementGroupPreviews = pgTable("intermediated_disbursement_group_previews", {
    ...
    `),
    check("intermediated_disbursement_group_previews_hash_check", sql`${table.previewHash} ~ '[^[:space:]]'`),
