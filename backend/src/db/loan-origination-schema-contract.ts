...
const loanConstraints: ContractEntry[] = [
    ...
    check("loans_floating_accrual_cycle_check", `CHECK ((repayment_type = 'floating' AND floating_accrual_cycle IN ('daily', 'weekly', 'monthly')) OR (repayment_type <> 'floating' AND floating_accrual_cycle IS NULL))`),
    ...
    check("loans_interest_period_unit_check", `CHECK (interest_period_unit IS NULL OR interest_period_unit IN ('day', 'week', 'month'))`),
