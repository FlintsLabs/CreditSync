...
describe("scheduled payment allocation correction migration", () => {
    test("registers the additive migration and immutable ledger constraints", async () => {
        ...
        expect(journal.entries.find((entry) => entry.tag === "0061_scheduled_payment_allocation_corrections")).toEqual({ idx: 61, version: "7", when: 1788811800000, tag: "0061_scheduled_payment_allocation_corrections", breakpoints: true });
