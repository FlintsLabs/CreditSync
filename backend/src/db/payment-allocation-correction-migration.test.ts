...
describe("scheduled payment allocation correction migration", () => {
    test("registers the additive migration and immutable ledger constraints", async () => {
        ...
        expect(journal.entries.find((entry) => entry.tag === "0062_scheduled_payment_allocation_corrections")).toEqual({ idx: 62, version: "7", when: 1788811800000, tag: "0062_scheduled_payment_allocation_corrections", breakpoints: true });
        const sql = await Bun.file(`${root}drizzle/0062_scheduled_payment_allocation_corrections.sql`).text();
