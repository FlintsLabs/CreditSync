...
const floatingInterestRate = t.String({ pattern: "^\\d+(?:\\.\\d{1,4})?$", maxLength: 32 });
const dailyInterestValue = t.String({ pattern: "^\\d+(?:\\.\\d{1,4})?$" });
...
export const dailyEntry = t.Object({ durationUnit: t.Union([t.Literal("days"), t.Literal("months")]), durationValue: t.Integer({ minimum: 1, maximum: 100_000 }), entryMode: t.Union([t.Literal("daily_payment"), t.Literal("daily_interest")]), dailyPayment: t.Optional(publicMoney), interestInput: t.Optional(t.Object({ mode: t.Union([t.Literal("percent"), t.Literal("fixed_amount"), t.Literal("per_thousand")]), value: dailyInterestValue }, { additionalProperties: t.Never() })) }, { additionalProperties: t.Never() });
