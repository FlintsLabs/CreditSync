...
export const toolDataSchemas: Record<McpToolName, z.ZodType<Record<string, unknown>>> = {
    ...
    "payment.identity-decision.preview": z.object({ identityDecisionPreviewPublicId: uuid, previewHash: z.string().regex(/^[0-9a-f]{64}$/i), participantSnapshotHash: z.string().regex(/^[0-9a-f]{64}$/i), participantPaymentIntakePublicIds: z.array(uuid), decision: z.enum(["same_payment", "distinct_payment"]), expiresAt: isoDateTime, auditPublicId: uuid, correlationId: uuid }).strict(),
...
const toolDescriptions: Record<McpToolName, string> = {
    ...
    "payment.identity-decision.preview": "Preview and audit an exact participant-scoped same-payment or distinct-payment identity decision.",
    "payment.identity-decision.execute": "Execute a confirmed identity decision after snapshot and evidence revalidation.",
    "payment.evidence-recovery.preview": "Preview an explicit evidence recovery successor while preserving the source requirement floor and provenance.",
    "payment.evidence-recovery.execute": "Execute a confirmed evidence recovery draft with immutable lineage and an idempotent receipt.",
