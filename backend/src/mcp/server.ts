...
const toolDataSchemas: Record<McpToolName, z.ZodType<Record<string, unknown>>> = {
    ...
    "payment.batch.workspace": z.object({ batchPublicId: uuid, batch: z.unknown(), items: z.array(z.unknown()) }).strict(),
    "payment.batch.staging.review": batchOutput,
    "payment.batch.staging.edit": z.object({ stagingItemPublicId: uuid, status: z.string(), revision: z.number().int() }).strict(),
    ...
    "payment.batch.decision": z.object({ decisionPublicId: uuid, batchPublicId: uuid, revision: z.number().int(), action: z.literal("confirm_no_older_pending") }).strict(),
    "payment.batch.cancel": batchOutput,
