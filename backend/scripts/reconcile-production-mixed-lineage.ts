...
async function verifyCatalogAndData(tx: postgres.TransactionSql, journalCount: 40 | 42 | 53 | 54, compareCaptured = false) {
...
async function run() {
  ...
  try {
    const result = await sql.begin(async (tx) => {
      ...
      if (![30, 40, 42, 53, 54].includes(count)) throw new Error(`mixed-lineage state is not exact: ${count} journal rows`);
      ...
      if (count === 54 || count === 53 || count === 42 || count === 40) { await verifyCatalogAndData(tx, count); return "already-complete"; }
