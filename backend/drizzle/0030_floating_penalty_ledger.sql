...
$$;
--> statement-breakpoint
CREATE CONSTRAINT TRIGGER "floating_transaction_allocations_totals_consistent"
AFTER INSERT ON "floating_transaction_allocations"
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION validate_floating_transaction_allocation_totals();
--> statement-breakpoint

CREATE OR REPLACE FUNCTION validate_floating_penalty_group_floor()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
	assessed_penalty numeric := 0;
	paid_penalty numeric := 0;
BEGIN
	SELECT COALESCE(SUM(ledger."amount"), 0)
	INTO assessed_penalty
	FROM "floating_penalty_ledger_entries" ledger
	...
	IF assessed_penalty < 0 OR assessed_penalty < paid_penalty THEN
		RAISE EXCEPTION 'floating penalty adjustment would reduce assessment below paid penalty';
	END IF;
	RETURN NULL;
END;
$$;
--> statement-breakpoint
CREATE CONSTRAINT TRIGGER "floating_penalty_group_floor_consistent"
AFTER INSERT ON "floating_penalty_ledger_entries"
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION validate_floating_penalty_group_floor();
--> statement-breakpoint
