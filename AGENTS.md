...
- Slip analysis must follow this order before creating or posting any payment:
  1. OCR every attached slip and normalize the Buddhist date to CE, keeping money as two-decimal strings. Report OCR uncertainty and do not persist raw OCR, QR payloads, account numbers, or full references.
  2. Search the sender by canonical name and confirmed aliases. If the sender is paying on behalf of another borrower, require an explicit human confirmation of that relationship before treating the borrower as identified.
  3. Identify the receiving profile and channel. Distinguish direct borrower-to-operator payments from intermediary collections and PromptPay/top-up destinations; do not assume that a displayed recipient is the borrower or the operator.
  4. Pull the complete accessible loan list for the identified borrower (including active, paid, renewed, restructured, and draft status where relevant), then inspect the authoritative contract and schedule for candidate loans. Do not match against a single loan chosen from name similarity alone.
  5. Compare the slip amount against both scheduled installment totals and backend-calculated interest. For floating loans, use the backend interest result; for scheduled loans, compare the exact due-date installment. Never recreate accounting calculations in the agent.
  6. Classify each slip as `exact_match`, `confirmed_on_behalf`, `grouped_interest_candidate`, `grouped_payment_candidate`, or `unmatched`. A grouped amount must be decomposed into explicit per-contract allocations whose decimal-string sum equals the slip amount; otherwise stop for review. Multiple plausible decompositions remain ambiguous.
  7. Check existing payment/collection/remittance history and idempotency before writing. A prior matching record or posted remittance means the slip must be treated as a duplicate until reviewed.
  8. Present the proposed contract mapping and totals for human confirmation. Only after confirmation may the agent create collection/intake records, then run the applicable preview. Post only a `ready` preview with zero variance, no warnings, current state, and complete allocation; otherwise stop and request review.
...
- Floating allocation integrity repair must start with `bun run backend/scripts/check-floating-allocation-integrity.ts` against the complete accessible floating-loan set. Treat `ACCRUAL_OVERALLOCATED`, `ALLOCATION_AFTER_ACCRUAL_FILLED`, `ACCRUAL_PAID_OVER_LIMIT`, or an active allocation targeting a reversed accrual as provenance corruption; a contractual advance-period `due_date` equal to the period end is not corruption by itself.
- For a confirmed grouped floating-payment repair, run `bun run backend/scripts/repair-floating-payment-batch.ts` in dry-run mode first with `TARGET_PAYMENT_INTAKE_PUBLIC_ID` and `PRECEDING_PAYMENT_INTAKE_PUBLIC_ID`. The repair must verify active floating loans, matching per-loan amounts, interest-only components, target accruals, and tenant ownership; then require explicit human confirmation before setting `EXECUTE_REPAIR=yes`.
- When transaction insertion order differs from Bangkok business-date order, reverse the older business-date batch first, reverse the affected later-date batch second, repost the older batch first, and repost the later batch second. Never edit or delete original posted records. Re-running with the same idempotency keys must resume existing replacement intakes instead of creating duplicates.
- After every repair, run `bun run backend/scripts/check-floating-allocation-integrity.ts` and `bun run backend/scripts/verify-floating-payment-repair.ts`; require zero integrity issues and health values consistent with accrual dates before declaring completion. If any ambiguity, downstream reversal dependency, stale state, or missing accrual occurs, stop for review.
...
- Keep the frozen MCP contract, plugin manifest/version, skills, eval scenarios, and validator synchronized whenever tools change. Read the current private plugin version from `plugins/creditsync/.codex-plugin/plugin.json`, skill inventory from its manifest/validator, and tool counts from `plugins/creditsync/references/mcp-profiles/index.json`; do not infer current versions or counts from historical release notes.
