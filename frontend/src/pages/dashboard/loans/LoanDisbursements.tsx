...
import { api, resolveFileAccessUrl } from "../../../lib/api";
...
export function LoanDisbursements({ loanPublicId, formatCurrency }: { loanPublicId: string; formatCurrency: (value: number) => string }) {
    ...
    const saveDraft = async () => {
        ...
        try {
            const body = { ...draft, grossAmount: Number(draft.grossAmount).toFixed(2), loanAttributedAmount: Number(draft.loanAttributedAmount).toFixed(2), sourceBankProfilePublicId: draft.sourceBankProfilePublicId || null, payeeHint: draft.payeeHint || null, note: draft.note || null, disbursedAt: new Date(draft.disbursedAt).toISOString() };
