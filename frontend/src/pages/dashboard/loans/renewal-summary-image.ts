...
export function buildRenewalSummarySvg(summary: LoanRenewalSummary, locale: string): string {
    ...
    const rows = labels.map((label, index) => `<text x="100" y="${400 + index * 62}" class="label">${escapeXml(label)}</text><text x="980" y="${400 + index * 62}" text-anchor="end" class="value" data-exact="${values[index]}">${money(values[index]!)}</text>`).join("");
    return `<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="1350" viewBox="0 0 1080 1350"><style>text{font-family:Sarabun,"Noto Sans Thai",Arial,sans-serif;fill:#10233f}.title{font-size:44px;font-weight:700}.watermark{font-size:34px;font-weight:700;fill:#b45309}.label,.small{font-size:25px}.value{font-size:29px;font-weight:700}.muted{font-size:21px;fill:#5b6778}</style><rect width="1080" height="1350" fill="#f4f8ff"/><rect x="55" y="55" width="970" height="1240" rx="34" fill="#fff" stroke="#d9e4f2" stroke-width="3"/><text x="100" y="130" class="title">CreditSync · ${th ? "สรุปการต่อสัญญา" : "Renewal summary"}</text><text x="100" y="188" class="watermark">${escapeXml(watermark)}</text><text x="100" y="250" class="label">${escapeXml(summary.borrower.displayName)}</text><text x="100" y="294" class="muted">${escapeXml(maskUuid(summary.oldContract.publicId))} · ${date(summary.oldContract.startDate, locale)} – ${date(summary.oldContract.dueDate, locale)}</text><line x1="100" y1="330" x2="980" y2="330" stroke="#d9e4f2" stroke-width="2"/>${rows}<line x1="100" y1="850" x2="980" y2="850" stroke="#d9e4f2" stroke-width="2"/><text x="100" y="828" class="muted">${th ? "ทิศทางเงินสด" : "Cash direction"}: ${escapeXml(c.cashDirection)}</text>${adjustmentRows}<text x="100" y="1140" class="label">${th ? "ประวัติรับชำระ" : "Payment history"}: ${c.payments.length}</text><text x="100" y="1184" class="muted">${paymentFirst ? date(paymentFirst, locale) : "—"} — ${paymentLast ? date(paymentLast, locale) : "—"}</text><text x="100" y="1245" class="muted">${th ? "สัญญาใหม่" : "Replacement"}: ${escapeXml(summary.replacement.publicId ? maskUuid(summary.replacement.publicId) : "—")} · ${money(summary.replacement.principal)}</text></svg>`;
}
