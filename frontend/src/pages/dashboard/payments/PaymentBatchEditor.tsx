...
function operationStorageKey(kind: "capture" | "execute" | "split", batchId: string | null) { return `${storageScope()}:${kind}-key:${batchId ?? "draft"}`; }
...
export function PaymentBatchEditor({ onPreview, onExecute }: PaymentBatchEditorProps) {
    ...
    useEffect(() => { localStorage.setItem(operationStorageKey("capture", batchPublicId), captureKey); localStorage.setItem(operationStorageKey("execute", batchPublicId), executeKey); localStorage.setItem(operationStorageKey("split", batchPublicId), splitKey); }, [batchPublicId, captureKey, executeKey, splitKey]);
    useEffect(() => {
    ...
    const captureFiles = async () => {
        ...
        try {
            ...
            const batchCaptureKey = localStorage.getItem(operationStorageKey("capture", staged.batchPublicId)) ?? captureKey; const batchExecuteKey = localStorage.getItem(operationStorageKey("execute", staged.batchPublicId)) ?? crypto.randomUUID(); const batchSplitKey = localStorage.getItem(operationStorageKey("split", staged.batchPublicId)) ?? crypto.randomUUID(); setCaptureKey(batchCaptureKey); setExecuteKey(batchExecuteKey); setSplitKey(batchSplitKey); localStorage.setItem(operationStorageKey("capture", staged.batchPublicId), batchCaptureKey); localStorage.setItem(operationStorageKey("execute", staged.batchPublicId), batchExecuteKey); localStorage.setItem(operationStorageKey("split", staged.batchPublicId), batchSplitKey); setBatchPublicId(staged.batchPublicId); localStorage.setItem(`${storageScope()}:batch-id`, staged.batchPublicId); setStep(2);
    ...
    const applyOcrProposal = (item: BatchItemDraft) => {
        ...
        update(item.id, { amount: item.ocrProposal.amount ?? item.amount, receivedAt: item.ocrProposal.transferredAt ? toBangkokDateTimeInput(item.ocrProposal.transferredAt) : item.receivedAt, payerName: item.ocrProposal.payerName ?? item.payerName });
    };
    ...
    return <Card data-testid="payment-batch-editor"><CardHeader><CardTitle>{t("paymentBatch.title")}</CardTitle><div className="grid grid-cols-2 gap-1 text-xs md:grid-cols-4" aria-label={t("paymentBatch.steps")}>{["stepCapture", "stepReview", "stepChronology", "stepConfirm"].map((label, index) => <span key={label} className={`rounded px-2 py-1 ${step === index + 1 ? "bg-slate-900 text-white" : "border"}`}>{index + 1}. {t(`paymentBatch.${label}`)}</span>)}</div></CardHeader><CardContent className="space-y-4">
        ...
        {items.map((item) => <div className="grid gap-2 rounded border p-3 md:grid-cols-4" key={item.id} data-testid="payment-batch-row">
            ...
            {!item.paymentIntakePublicId && item.stagingItemPublicId && item.evidenceStatus === "ready" && <Button type="button" variant="outline" disabled={busy} onClick={() => void extractProposal(item)}>{t("paymentBatch.extractCandidates")}</Button>}{item.ocrProposal && <div className="rounded bg-slate-50 p-2 text-xs md:col-span-4"><p>{t("paymentBatch.ocrReviewOnly")}</p><p>{t("paymentBatch.ocrAmount")}: {item.ocrProposal.amount ?? "—"} · {t("paymentBatch.ocrTime")}: {item.ocrProposal.transferredAt ? toBangkokDateTimeInput(item.ocrProposal.transferredAt) : "—"} · {t("paymentBatch.payerName")}: {item.ocrProposal.payerName ?? "—"} · {t("paymentBatch.ocrReceiver")}: {item.ocrProposal.receiverName ?? "—"} · {t("paymentBatch.ocrFee")}: {item.ocrProposal.fee ?? "—"}</p><Button type="button" variant="outline" disabled={busy} onClick={() => applyOcrProposal(item)}>{t("paymentBatch.useOcrCandidate")}</Button></div>}{item.paymentIntakePublicId && <Button type="button" variant="outline" disabled={busy} onClick={() => void editReviewedItem(item)}>{t("paymentBatch.editReviewed")}</Button>}{item.uploadStatus === "failed" && <Button type="button" variant="outline" disabled={busy} onClick={() => void captureFiles()}>{t("paymentBatch.retryFile")}</Button>}{item.error && <p role="alert" className="text-sm text-red-700">{item.error}</p>}
        </div>)}
