...
interface IdentityPreview { identityDecisionPreviewPublicId: string; previewHash: string; decision: "same_payment" | "distinct_payment"; participantPaymentIntakePublicIds: string[]; participantSnapshots: Array<{ publicId: string; amount: string; payerName: string | null; receivedAt: string; timeDifferenceSeconds: number }>; expiresAt: string }
        ...
        <div className="grid gap-5 xl:grid-cols-[0.85fr_1.4fr]">
            ...
            {detailLoading ? <Card><CardContent role="status" className="py-16 text-center text-muted-foreground">{t("payments.loadingDetail")}</CardContent></Card> : !detail ? <Card><CardContent className="py-16 text-center text-muted-foreground">{t("payments.select")}</CardContent></Card> : <div className="space-y-5">
                <Card><CardHeader><CardTitle className="flex justify-between"><span>{t("payments.review")}</span><Badge>{t(`payments.status.${detail.status}`)}</Badge></CardTitle></CardHeader><CardContent className="space-y-4">
                    ...
                    {identityPreview && <div className="rounded border border-sky-300/50 bg-sky-50 p-3 text-sm dark:bg-sky-950/30"><p>{t("payments.identity.confirm", { decision: t(`payments.identity.${identityPreview.decision === "same_payment" ? "same" : "distinct"}`) })}</p><div className="mt-2 space-y-1" aria-label={t("payments.identity.participantsLabel")}>{identityPreview.participantSnapshots.map((participant) => <div className="flex flex-wrap justify-between gap-2" key={participant.publicId}><span>{participant.payerName ?? t("payments.identity.unknownPayer")} · {money(participant.amount)}</span><span>{dateTime(participant.receivedAt)} · {t("payments.identity.timeDifference", { seconds: participant.timeDifferenceSeconds })}</span></div>)}</div><label className="mt-2 flex gap-2"><input type="checkbox" checked={recoveryConfirmed} onChange={(event) => setRecoveryConfirmed(event.target.checked)} />{t("payments.identity.confirmCheckbox")}</label><Button className="mt-2" size="sm" disabled={busy || !recoveryConfirmed} onClick={() => executeIdentity()}>{t("payments.identity.execute")}</Button></div>}
                    ...
                    {!!detail.evidence?.length && detail.evidence.map((item, index) => {
                        ...
                        return <div key={item.publicId} className="flex flex-wrap items-center justify-between gap-2 rounded bg-muted/40 p-2 text-xs">
                            <span>{item.mimeType} · {t(`payments.status.${item.status}`)} · <span className="font-mono">{item.publicId}</span></span>
                            {previewAvailable
                                ? <EvidencePreviewButton available label={`${t("evidence.previewSlip")} ${index + 1}`} mimeType={item.mimeType} resolve={() => resolveFileAccess(item.filePublicId!)} />
                                : <span role="status" className="basis-full text-amber-700 dark:text-amber-300">{t("evidence.pendingSlip")}</span>}
                        </div>;
                    })}
                </CardContent></Card>
